...
// The Afmt library version in the form major * 10000 + minor * 100 + patch.
...
// Define FMT_USE_NOEXCEPT to make Afmt use noexcept (C++11 feature).
...
/**
  ...
  subset of the API. ``Afmt::basic_string_view`` is used for format strings even
...
/**
  ...
  {Afmt} provide an overload of `to_string_view` for it in the same namespace as
  ...
  **Example**::
    ...
    std::string message = Afmt::format(my_string("The answer is {}"), 42);
  \endrst
 */
...
// A base class for compile-time strings. It is defined in the Afmt namespace to
// make formatting functions visible via ADL, e.g. format(Afmt("{}"), 42).
...
// Specifies whether S is a string type convertible to Afmt::basic_string_view.
...
template <typename Context> struct arg_mapper {
  ...
  int map(...) {
    ...
    static_assert(
        ...
        "https://Afmt.dev/latest/api.html#formatting-user-defined-types");
    return 0;
  }
};
...
// A type-erased reference to an std::locale to avoid heavy <locale> Afmt.
class locale_ref {
 private:
...
/**
  ...
  `~Afmt::basic_format_args` for passing into type-erased formatting functions
  such as `~Afmt::vformat`.
  \endrst
 */
...
/**
  \rst
  Constructs an `~Afmt::format_arg_store` object that contains references to
  arguments and can be implicitly converted to `~Afmt::format_args`. `Context`
  can be omitted in which case it defaults to `~Afmt::context`.
  See `~Afmt::arg` for lifetime considerations.
  \endrst
 */
...
/**
  \rst
  A dynamic version of `Afmt::format_arg_store<>`.
  It's equipped with a storage to potentially temporary objects which lifetime
  could be shorter than the format arguments object.

  It can be implicitly converted into `~Afmt::basic_format_args` for passing
  into type-erased formatting functions such as `~Afmt::vformat`.
  \endrst
 */
 ...
 public:
  /**
    ...
    **Example**::
      ...
      Afmt::dynamic_format_arg_store<Afmt::format_context> store;
      store.push_back(42);
      store.push_back("abc");
      store.push_back(1.5f);
      std::string result = Afmt::vformat("{} and {} and {}", store);
    \endrst
  */
...
template <typename Context> class basic_format_args {
 ...
 public:
  ...
  /**
   \rst
   Constructs a `basic_format_args` object from `~Afmt::format_arg_store`.
   \endrst
   */
...
/**
  ...
  **Example**::
    ...
    Afmt::print("Elapsed time: {s:.2f} seconds", Afmt::arg("s", 1.23));
  \endrst
 */
...
/**
  ...
  **Example**::
    ...
    #Afmt <Afmt/core.h>
    std::string message = Afmt::format("The answer is {}", 42);
  \endrst
*/
...
/**
  ...
  **Example**::
    ...
    Afmt::print("Elapsed time: {0:.2f} seconds", 1.23);
  \endrst
 */
template <typename S, typename... Args, typename Char = char_t<S>>
